...
class ControllChat:
    def __init__(self,view : ViewServer,port):
        self.view = view
        self.port = port
        self.isSend = False
    ...
    def run(self):
        ...
        self.sock.listen(1) 
        self.view.addMessage("---------Aguardando conexao---------")
        self.connection, self.address = self.sock.accept()
        self.isSend = True
        self.view.addMessage(f"Conectado com {self.address}")
        mensagem = b''
    
        try:
            while self.IsClose == True:
                mensagem = self.connection.recv(1024)
                if not mensagem:
                    self.sock.close()
                    break
...
def send_message():
    if controll.isSend: 
        controll.connection.send(f"SERVER : {controll.view.entryMessage.get()}".encode())
        controll.view.addMessage(f"SERVER : {controll.view.entryMessage.get()}")
        controll.view.entryMessage.delete(0,customtkinter.END)

if __name__ == "__main__":
    view = ViewServer()
    controll = ControllChat(view,customtkinter.CTkInputDialog(title="Informe a porta : ",text="Informe a porta : ").get_input())
    view.draw()
